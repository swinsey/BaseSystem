...
import static de.silveryard.basesystem.sdk.kernel.bluetooth.phone.BtPhoneManager.systemCallDriverBtPhoneManagerGetDevices;


/**
 * Created by silveryard on 30.05.17.
 */
public abstract class BluetoothPhoneDriver {
    private static final Wrapper<ReturnCode> returnCodeWrapper = new Wrapper<>();
    private static final Wrapper<BtPhoneReturnCode> btPhoneReturnCodeWrapper = new Wrapper<>();
    ...
    private static final List<PhoneDevice> devices = new ArrayList<>();

    /**
     * Returns all currently known audio devices
     * @return
     */
    public static synchronized List<PhoneDevice> getDevices() {
        return devices;
    }
    ...
    public static synchronized void updateDevices() {
        systemCallDriverBtPhoneManagerGetDevices(returnCodeWrapper, btPhoneReturnCodeWrapper, intArrayWrapper);
        ...
        for (int i = 0; i < intArrayWrapper.value.length; i++) {
            ...
            if (!alreadyConnected) {
                PhoneDevice device = new PhoneDevice(intArrayWrapper.value[i]);
                devices.add(device);
