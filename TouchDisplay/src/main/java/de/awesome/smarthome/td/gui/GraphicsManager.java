...
import de.awesome.smarthome.td.util.SortedList;

import java.util.ArrayList;
import java.util.List;

import static de.awesome.smarthome.td.gui.SDLWindow.*;
...
public class GraphicsManager implements IDisposable{
    ...
    public static void initialize(String title, int width, int height, boolean showCursor){
        ...
        int result = windowInit(title, width, height, showCursor);
        if(result != 0){
    ...
    private void handleEvents(){
        while (windowPollEvent() != 0) {
            ...
            if(type == SDLEventTypes.SDL_QUIT) {
                ...
                if(mouseDown){
                    int x = windowGetMouseX();
                    int y = windowGetMouseY();
    ...
    private void renderInternal(){
        ...
        //Clear screen
        windowSetDrawColor((byte)0xFF, (byte)0xFF, (byte)0xFF, (byte)0xFF);
        windowClear();
